...
func (c *uRLServiceClient) IncrementClicks(ctx context.Context, in *IncrementClicksRequest, opts ...grpc.CallOption) (*IncrementClicksResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(IncrementClicksResponse)
	err := c.cc.Invoke(ctx, URLService_IncrementClicks_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func _URLService_IncrementClicks_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(IncrementClicksRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(URLServiceServer).IncrementClicks(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: URLService_IncrementClicks_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(URLServiceServer).IncrementClicks(ctx, req.(*IncrementClicksRequest))
	}
	return interceptor(ctx, in, info, handler)
}
